...
class DBManager:
    ...
    def get_companies_and_vacancies_count(self) -> list[tuple[Any]]:
        ...
        conn = psycopg2.connect(dbname='hh_info', **DBManager.params)
    ...
    def get_vacancies_with_higher_salary(self) -> list[tuple[Any]]:
        ...
        with conn.cursor() as cur:
            cur.execute("SELECT employers.employer_name, vacancies.vacancy_name, vacancies.salary_from, "
                        "vacancies.salary_to, vacancies.vacancy_url FROM vacancies JOIN employers USING(employer_id) "
                        "GROUP BY employers.employer_name, vacancies.vacancy_name, vacancies.salary_from, "
                        "vacancies.salary_to, vacancies.vacancy_url HAVING vacancies.salary_from > "
                        "(SELECT AVG(salary_from) FROM vacancies WHERE salary_from <> 0) "
